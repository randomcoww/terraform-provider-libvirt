...
func providerConfigure(d *schema.ResourceData) (interface{}, error) {
	...
	if err := writeFile(path.Join(tempDir, "clientcert.pem"), d.Get("client_cert").(string)); err != nil {
		return nil, err
	}
	if err := writeFile(path.Join(tempDir, "clientkey.pem"), d.Get("client_key").(string)); err != nil {
		return nil, err
	}
